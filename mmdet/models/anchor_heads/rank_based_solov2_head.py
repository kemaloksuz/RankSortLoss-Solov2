...
from mmdet.core import multi_apply, matrix_nms, vectorize_labels
...
class RankBasedSOLOv2Head(nn.Module):
    ...
    def __init__(self,
                 ...
                 type_dcn=None,
                 rank_loss_type = 'aLRP'):
        super(RankBasedSOLOv2Head, self).__init__()
    ...
    def loss(self,
        ...
        if num_ins > 0:
            flattened_ins_pred = torch.cat([torch.cat([ins_pred for ins_pred in ins_pred_list if ins_pred is not None])])
            flattened_ins_pred = torch.sigmoid(flattened_ins_pred)
            flattened_ins_labels = torch.cat([torch.cat([ins_label for ins_label in ins_labels if len(ins_label) != 0])])
            loss_instances = dice_loss(flattened_ins_pred, flattened_ins_labels)
            ...
            ins_weights = flatten_cate_preds.detach().sigmoid().max(dim=1)[0][flatten_ins_ind_labels]

            flat_labels = vectorize_labels(flatten_cate_labels, self.num_classes - 1)
            flat_labels[flat_labels==1] = 1-loss_instances.detach()

            flat_preds = flatten_cate_preds.reshape(-1)
            if self.rank_loss_type == 'aLRP':
                loss_cls, rank, order = self.loss_rank.apply(flat_preds, flat_labels, loss_instances)

                #Order the regression losses considering the scores. 
                ordered_losses_ins = loss_instances[order.detach()].flip(dims=[0])
            
                #Compute aLRP Regression Loss
                loss_ins = ((torch.cumsum(ordered_losses_ins,dim=0)/rank[order.detach()].detach().flip(dims=[0])).mean())
                
                self.cls_LRP_hist.append(float(loss_cls.item()))
                self.reg_LRP_hist.append(float(loss_ins.item()))
                self.counter+=1
                ...
                if self.counter == self.period:
                    ...
                    self.counter=0
                loss_ins *= self.SB_weight

            elif self.rank_loss_type == 'RankSort':
                ranking_loss, sorting_loss = self.loss_rank.apply(flat_preds, flat_labels)

                ins_avg_factor = torch.sum(ins_weights)
                if ins_avg_factor < 1e-10:
                    ins_avg_factor = 1
                
                loss_ins = torch.sum(ins_weights*loss_instances)/ins_avg_factor
                SB_weight = (ranking_loss+sorting_loss).detach()/float(loss_ins.item())
                loss_ins *= SB_weight
                return dict(loss_rank=ranking_loss, loss_sort=sorting_loss, loss_ins=loss_ins)


        else:
            loss_ins = torch.cat(ins_pred_list).sum() * 0
            if self.rank_loss_type == 'RankSort':
                ranking_loss = torch.cat(cate_preds).sum() * 0
                sorting_loss = torch.cat(cate_preds).sum() * 0
                return dict(loss_rank=ranking_loss, loss_sort=sorting_loss, loss_ins=loss_ins)

            loss_cls = torch.cat(cate_preds).sum() * 0
    ...
    def solov2_target_single(self,
        ...
        for (lower_bound, upper_bound), stride, num_grid \
            ...
            for seg_mask, gt_label, half_h, half_w in zip(gt_masks, gt_labels, half_hs, half_ws):
                ...
                idx =  (cate_label[top:(down+1), left:(right+1)] == 0)
                cate_label[top:(down+1), left:(right+1)][idx] = gt_label
